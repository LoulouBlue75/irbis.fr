export default function ApproachPage() {
  return (
    <div className="flex flex-col w-full">
      {/* HERO */}
      <section className="bg-ivory min-h-[50vh] flex flex-col items-center justify-center text-center px-6 py-20">
        <h1 className="text-display mb-6">
          Method. Heart.
        </h1>
        <p className="text-xl text-text-secondary font-medium">
          Structure that respects complexity.
        </p>
      </section>

      {/* PILLARS */}
      <section className="bg-white py-24 px-6">
        <div className="container mx-auto max-w-5xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-16">
            {[
              { num: "01", title: "Discipline", sub: "Rigour without rigidity." },
              { num: "02", title: "Presence", sub: "Nuance. Human reality." },
              { num: "03", title: "Illumination", sub: "Signal. Trade-offs. Clarity." },
              { num: "04", title: "Elevation", sub: "Standards pulled upward." },
            ].map((pillar) => (
              <div key={pillar.num} className="flex flex-col gap-6 group">
                <div className="flex items-baseline justify-between border-b border-border-primary pb-4 group-hover:border-accent-primary transition-colors duration-300">
                  <span className="text-sm font-bold text-gold tracking-wider">{pillar.num}</span>
                  <h3 className="text-2xl font-serif font-semibold">{pillar.title}</h3>
                </div>
                <p className="text-lg text-text-secondary leading-relaxed">{pillar.sub}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* CONFIDENTIALITY */}
      <section className="bg-ivory py-24 px-6 text-center">
        <div className="container mx-auto max-w-2xl">
          <h2 className="text-h2 mb-8">
            Confidential by design.
          </h2>
          <p className="text-lg text-text-secondary leading-relaxed">
            Visibility on match only. Salaries masked. Identities protected.
          </p>
        </div>
      </section>
    </div>
  );
}
